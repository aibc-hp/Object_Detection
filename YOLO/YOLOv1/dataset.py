...
class YoloDataset(Dataset):
    ...
    def encoder(self, boxes, labels):
        num_grid = 14  # 最终输出的特征图一行或一列的单元格数量
        target = torch.zeros((num_grid, num_grid, 30))  # 初始化一个大小为 (num_grid, num_grid, 30) 的张量，用于存储每个单元格的目标信息；30 表示每个单元格对应的 2 个预测框信息和置信度以及 20 个类别的概率
        cell_size = 1. / num_grid  # 单元格大小
...
def main():
    ...
    train_loader = DataLoader(train_dataset, batch_size=1, shuffle=False, num_workers=0, drop_last=False)
